...
def validate_stage_transition(
    ...
    # Check ordering if strict_order is enabled
    gating = config.get('gating', {})
    if gating.get('strict_order', False):
        if current_stage:
            ...
            # Check for skipping (only allow adjacent stages)
            if target_idx > current_idx + 1:
                return {
                    'valid': False,
                    'message': f"Cannot skip stages. Next: {stage_ids[current_idx + 1]}"
                }
